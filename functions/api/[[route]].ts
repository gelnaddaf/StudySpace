...
app.get('/notes/:noteId/files', async (c) => {
  const db = c.env.DB
...
app.post('/notes/:noteId/files', async (c) => {
  const db = c.env.DB
...
// Extract LOs from text content
app.post('/ai/extract-los', async (c) => {
  const ai = c.env.AI
...
// Suggest note-to-LO links
app.post('/ai/suggest-links', async (c) => {
  const ai = c.env.AI
  const db = c.env.DB
...
// Gap analysis
app.post('/ai/gap-analysis', async (c) => {
  const ai = c.env.AI
  const db = c.env.DB
...
export const onRequest: PagesFunction<{ DB: D1Database; BUCKET: R2Bucket; AI: Ai }> = async (context) => {
